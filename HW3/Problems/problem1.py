...
#----------------------------------------------------
'''
    (Load Webpage Data) Given a file name of a CSV file containing all the webpages, load the CSV file into a pandas dataframe. Each row 
    contains information from a webpage. The data frame has 4 columns: ID, URL, Title and Description. 
    ---- Inputs: --------
...
#----------------------------------------------------
'''
    (Count Word Frequency) Given the dataframe X, compute the frequency of the keywords in each webpage's 'Description' and 
    add a column (called 'Count') to the dataframe X to store the frequencies of the searched keyword. 
	 ...
	 Then the result dataframe X should be as follows: 
	  ...
	  ----------------------- 
	    ...
	    2   | ... | ...| 0 
	  ----------------------- 
	 Note, the keyword matching is not case-sensitive. So we assume that all words in the webpages should be converted to lower cases before 
     being matched with the keyword. For example, in webpage 0, the 'Thanksgiving' should be first converted to lower cases 'thanksgiving', 
     then it will match with the keyword 'thanksgiving' and being counted. 
    ---- Inputs: --------
        * X: a dataframe of all the webpages, where each row represents a webpage.
    ...
    ---- Outputs: --------
        * X: a dataframe of all the webpages, where each row represents a webpage.
    ---- Hints: --------
        * In a pandas Series x (which is a column of a dataframe X), you could find various functions for x.str to process string values, 
            for example if a function is named f(), you could call the function by x.str.f(). 
        * To ignore the cases in word count, you could use a flag value in the "re" package as the parameter for the above f() function. 
            Here "re" is designed for regular expression. 
        * This problem can be solved using 1 line(s) of code.
'''
#---------------------
def count_word_frequency(X, keyword):
    #########################################
    ## INSERT YOUR CODE HERE (5 points)
    X.insert(loc = len(X.columns), column = 'Count', value = X['Description'].str.count('Thanksgiving|thanksgiving'))
    #########################################
    return X
    #-----------------
    '''  
