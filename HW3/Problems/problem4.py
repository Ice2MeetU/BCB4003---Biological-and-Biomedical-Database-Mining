...
'''
    ...
    In this problem, you will implement the ranking algorithm for webpages using PageRank (random walk with restart on the revised 
    hyperlink graph to solve both sink node and sink region problems). 
 ------------------------------------ 
      Search Engine (Version 3) 
 ------------------------------------ 
 In this version of the search engine, we implement use random walk method on a revised hyperlink graph to address both sink node and 
 sink region problem. (1) If a webpage has no hyperlink (which is a sink node), add a hyperlink from this webpage to all the webpages 
 in the network (including itself); (2) for every step of the random walk, the user will have a small probability to jump to a random 
 webpage ignoring the hyperlinks and a large probability to follow the hyperlink
...
#----------------------------------------------------
'''
    Given an adjacency matrix A of a webpage hyperlink network, compute the count of out-links (also called 'degree') for each webpage 
    in the network. 
    ---- Inputs: --------
        * A: the adjacency matrix indicating the hyperlinks between webpages, a numpy matrix of shape (n X n). A[i,j] =1, if there is 
            a hyperlink from webpage j to webpage i; A[i][j]=0, if there is no hyperlink from webpage j to webpage i.
    ---- Outputs: --------
        * d: the vector storing the out-link degrees of all webpages, a numpy vector of length n. d[i] represents the out-link degree 
            of webpage i, which is the number of hyperlinks on webpage i.
    ---- Hints: --------
...
#----------------------------------------------------
'''
    Given an adjacency matrix A (containing sink nodes), find all sink nodes in the network, then revise the adjacency matrix A by adding
    a hyperlink from each sink node to all the webpages (including itself). 
    ---- Inputs: --------
        * A: the adjacency matrix indicating the hyperlinks between webpages, a numpy matrix of shape (n X n). A[i,j] =1, if there is a 
            hyperlink from webpage j to webpage i; A[i][j]=0, if there is no hyperlink from webpage j to webpage i.
    ---- Outputs: --------
        * A: the adjacency matrix indicating the hyperlinks between webpages, a numpy matrix of shape (n X n). A[i,j] =1, if there is a 
            hyperlink from webpage j to webpage i; A[i][j]=0, if there is no hyperlink from webpage j to webpage i.
    ---- Hints: --------
...
'''
    ...
    ---- Inputs: --------
        * A_: the adjacency matrix indicating the hyperlinks between webpages (where all the sink nodes are fixed), a numpy matrix of shape 
            (n X n). A_[i,j] =1, if there is a hyperlink from webpage j to webpage i; A_[i][j]=0, if there is no hyperlink from webpage j 
            to webpage i.
    ---- Outputs: --------
        * S: the probability transition matrix among webpages (after solving the sink node problem), a (n by n) numpy matrix.  S[i,j] represents 
            the probability of a random-walk user moving from webpage j to webpage i by clicking a hyperlink on webpage j. The values in each 
            column of matrix S should sum to 1.
    ---- Hints: --------
...
#----------------------------------------------------
'''
    Given the probability transition matrix S of a webpage hyperlink network, fix the sink region problem in the probability transition matrix by
    allowing the random walk to have a large probability (alpha) to follow the hyperlinks and a small probability (1-alpha) to restart 
    (jump to a random webpage, including itself, ignoring hyperlinks). 
    ---- Inputs: --------
        * S: the probability transition matrix among webpages (after solving the sink node problem), a (n by n) numpy matrix.  
            S[i,j] represents the probability of a random-walk user moving from webpage j to webpage i by clicking a hyperlink on webpage j.
    ...
    ---- Outputs: --------
        * G: the probability transition matrix of the hyperlink network (after fixing sink nodes and sink regions), a (n by n) numpy matrix 
            of float values.  G[i,j] represents the probability of a user moving from webpage j to webpage i either by clicking a hyperlink 
            or randomly jumping to webpage j. The values in each column of matrix G should sum to 1.
    ---- Hints: --------
...
def compute_G(S, alpha):
    ...
    G = alpha * S + (1 - alpha)*(np.ones(S.shape)/len(S[0]))
    #########################################
    return G
    #-----------------
    '''  
...
#----------------------------------------------------
'''
    Given two vectors of the webpage scores, check whether or not all the values in the two vectors are close enough. For example, 
    if the tolerance (tol) is 0.1, the two values a and b are called 'close' if the absolute difference between the two values |a-b|<=0.1. 
    Now when we have two vectors of values x, y, the two vectors are 'close' if all values of the two vectors are 'close'. 
    ---- Inputs: --------
        * x: the vector of probability distribution of the random walker on all webpages in the network, a numpy vector of length n. x[i] represents the probability of the random walker in Webpage i.
        * y: .
    ...
    --------------------------------------
    ---- Example: --------
        For example, suppose we have two vectors x=[1.05, 4.03], y=[1.02, 4.05], if the tolerance is 0.1, then x and y are 'close'. In this case,
         the returned result (c) should be True
        For example, suppose we have two vectors m=[1.05, 4.03, 0.16], n=[1.02, 4.05, 0.35], if the tolerance is 0.1, then m and n are
         NOT 'close', because the last element is not close to each other within the tolerance. In this case, the returned result (c) 
         should be False 
    --------------------------------------
    ---- Hints: --------
...
#----------------------------------------------------
'''
    Given the probability transition matrix G of a webpage network and a vector of the initial probability distribution (x) on all the webpages,
    compute the stationary probability distribution on the webpages (also called 'pagerank scores') and return them as vector x. 
	 ...
	 (2) for each step inside the for-loop, 
		  ...
		  (2.2) check whether the scores before the random walk (x) and the scores after the random walk (y) are close enough within the 
            tolerance (tol) 
		  (2.3) if the two score vectors are close, break the for-loop, otherwise update the score vector x with the new vector y. 
    ---- Inputs: --------
        * G: the probability transition matrix of the hyperlink network (after fixing sink nodes and sink regions), a (n by n) numpy matrix of 
            float values.  G[i,j] represents the probability of a user moving from webpage j to webpage i either by clicking a hyperlink or 
            randomly jumping to webpage j. The values in each column of matrix G should sum to 1.
        * x: the vector of probability distribution of the random walker on all webpages in the network, a numpy vector of length n. x[i]  
            represents the probability of the random walker in Webpage i.
        * tol: the tolerance level, a float scalar, if all elements in two vectors x and y are close (|x[i]-y[i]|<= tol), then the x and y 
            are close to each other.
        * max_steps: the maximum number of random walk steps, an integer scalar.
    ---- Outputs: --------
        * x: the vector of probability distribution of the random walker on all webpages in the network, a numpy vector of length n. 
            x[i] represents the probability of the random walker in Webpage i.
    ---- Hints: --------
        * You could use the 'break' statement in python to break a for-loop. 
        * This function will take less than 1 second to run, if this function runs for a very long time without stopping, it means that the program 
            never encountered the 'break' statement inside the for-loop. 
...
#----------------------------------------------------
'''
    Given the pagerank scores of each webpage in (x) and the dataframe storing all the webpage data (X2), add a new column called 'PageRank'
    to dataframe (X2) to store the PageRank scores for all webpages. 
    ---- Inputs: --------
...
def add_column_pagerank(X2, x):
    #########################################
    ## INSERT YOUR CODE HERE (5 points)
    X2.insert(loc = len(X2.columns), column = 'PageRank', value = x)
    #########################################
    return X2
    #-----------------
    '''  
...
#----------------------------------------------------
'''
    (Rank Webpages by PageRank Score) Given a dataframe (X3) of all the webpages, rank all the pages based upon descending order of pagerank 
    scores (PageRank). 
    ---- Inputs: --------
        * X3: a dataframe of all the webpages containing the columns of pagerank socres, word frequency and number of inlinks, where each row 
