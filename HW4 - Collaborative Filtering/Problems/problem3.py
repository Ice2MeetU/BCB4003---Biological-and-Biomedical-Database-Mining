...
#----------------------------------------------------
'''
    (Extract Training Data for User j) Given the rating vector of the j-th User (R_j) and the hidden factors of all items (I), 
    extract a training dataset for User j, by excluding the missing ratings. 
	  There can be missing values in the rating vector (R_j), 
	      When R_j[i] is NaN (missing), which means that the user j has not yet rated the i-th item, so we should exclude the i-th item 
	  ...
	  Now the labels of the linear regression are the ratings of the two items: 
	      y = [1,2] . 
    ---- Inputs: --------
        * R_j: the ratings of all items by the j-th user (including missing ratings), a float numpy vector of length m, where R_j[i] is the 
            rating of j-th user on the i-th item, which could be a float scalar or a NaN (Not a Number, missing rating).
    ...
    ---- Outputs: --------
        * X: the feature matrix of the training samples, where each row X[i] represents the feature vector for the i-th training sample in the
             training dataset.
...
#----------------------------------------------------
'''
    (Train Ridge Regression Model for User j) Given training set for the j-th User (X, y), train a ridge regression model to get the 
    hidden factors of User j (Uj), which is the weights w of the ridge regression model. 
    ---- Inputs: --------
        * X: the feature matrix of the training samples, where each row X[i] represents the feature vector for the i-th training sample in 
...
def train_user_j(X, y, a=1e-05):
    #########################################
    ## INSERT YOUR CODE HERE (7 points)
    Uj = (X.T@y)@(np.linalg.inv(X.T@X+a*np.identity(X.shape[1])))
    #########################################
    return Uj
    #-----------------
    '''  
...
#----------------------------------------------------
'''
    (Train Ridge Regression Models for All Users) Given the rating matrix for all Users (R) and the hidden factors of all items (I), 
    train the hidden factors for all users (U) by extracting a training dataset and training a ridge regression model on each user. 
	 ...
	 Now we can put together the hidden factors for both users: 
	      U = [U1, = [[ 10, 0 ], 
	           U2]    [-60, 55]] . 
    ---- Inputs: --------
        * R: the ratings of all items by all users (including missing ratings), a float numpy matrix of shape (m X n), where R[i,j] is the 
            rating of j-th user on the i-th item, which could be a float scalar or a NaN (Not a Number, missing rating).
        * I: the hidden factors of all items, a float numpy matrix of shape (m X k), where I[i,j] is the j-th hidden factor of the i-th item.
        * a: (alpha) the weight for the regularization term on the hidden factors (to shrink their values), a float scalar.
    ---- Outputs: --------
...
#----------------------------------------------------
'''
    (Extract Training Data for Item j) Given the rating vector of the i-th Item (Ri_) and the hidden factors of all users (U), extract a training
     dataset for Item i, by excluding the missing ratings. 
	  There can be missing values in the rating vector (Ri_), 
	      When Ri_[j] is NaN (missing), which means that the user j has not yet rated the i-th item, so we should exclude the j-th user from the
           training dataset; 
	  ...
	  Now the labels of the linear regression are the ratings of the two users: 
	      y = [1,2] . 
    ---- Inputs: --------
        * Ri_: the ratings of the i-th item by all users (including missing ratings), a float numpy vector of length m, where Ri_[j] is the 
            rating of j-th user on the i-th item, which could be a float scalar or a NaN (Not a Number, missing rating).
        * U: the hidden factors of all users, a float numpy matrix of shape (n X k), where U[i,j] is the j-th hidden factor of the i-th user.
    ---- Outputs: --------
        * X: the feature matrix of the training samples, where each row X[i] represents the feature vector for the i-th training sample in the 
...
def update_I(R, U, a=1e-05):
    #########################################
    ## INSERT YOUR CODE HERE (7 points)
    I = np.empty((R.shape[0], U.shape[1]))
    for i in range(R.shape[0]):
        X, y = extract_user_j(R[i, :], U)
        I[i] = train_user_j(X, y, a)
    #########################################
    return I
    #-----------------
    '''  
...
#----------------------------------------------------
'''
    (Collaborative Filtering by Matrix Decomposition) Given the rating matrix R (with missing values), compute the hidden factors of items (I) 
    and users (U) using alternative coordinate descent. . 
    ---- Inputs: --------
        * R: the ratings of all items by all users (including missing ratings), a float numpy matrix of shape (m X n), where R[i,j] is the 
            rating of j-th user on the i-th item, which could be a float scalar or a NaN (Not a Number, missing rating).
