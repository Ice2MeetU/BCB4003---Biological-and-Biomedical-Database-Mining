...
#----------------------------------------------------
'''
    (Linear Regression with Least Square Loss) Let's start with a simpler method (called 'Least Square'). Given a set of training samples (X) 
    and their labels (y), train a linear model (with parameters w) on training samples using least-square regression. 
    ---- Inputs: --------
        * X: the feature matrix of the training samples, a numpy matrix of shape n by p, where X[i] represents the p-dimensional feature 
        vector for the i-th training sample in the training dataset.
...
#----------------------------------------------------
'''
    (Ridge Regression) Now let's build the ridge regression by adding a regularization term to the least square loss.   Given a set of 
    training samples (X) and their labels (y), train a linear model (with parameters w) on training samples using ridge regression 
    (i.e., least square regression with L2 regularization). 
    ---- Inputs: --------
        * X: the feature matrix of the training samples, a numpy matrix of shape n by p, where X[i] represents the p-dimensional feature 
            vector for the i-th training sample in the training dataset.
        * y: the labels, a numpy vector of length n, y[i] represents the label of the i-th training sample in the training dataset.
...
def ridge_regression(X, y, a=0.0001):
    #########################################
    ## INSERT YOUR CODE HERE (10 points)
    w = (X.T@y)@(np.linalg.inv(X.T@X+a*np.identity(X.shape[1])))
    #########################################
    return w
    #-----------------
    '''  
