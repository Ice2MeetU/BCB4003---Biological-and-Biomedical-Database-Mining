...
#----------------------------------------------------
'''
    Create an identity matrix of shape n X n, all the values in the matrix should be 0, except the diagonal elements (which should be 1). 
    For example, an identity matrix of shape 3 X 3 will be 
	 1, 0, 0 
	 0, 1, 0 
	 0, 0, 1. 
    ---- Inputs: --------
...
#----------------------------------------------------
'''
    Given a matrix X (of shape r X c), then compute the inverse matrix of X (called Xi), so that  X Xi = I (identity matrix) 
 For example, if matrix X is: 
	 2, 0 
	 0, 4 
...
#----------------------------------------------------
'''
     Given an input numpy vector x and a boolean vector (m) of the same length as x, extract the subset of elements in x indexed by m 
     (where the elements to be used are True in m, the elements to be discarded are False in m).
...
#----------------------------------------------------
'''
     Given an input numpy matrix X and a boolean vector (m), extract the subset matrix in X indexed by m (where the rows of X to be 
     used are True in m, the rows of X to be discarded are False in m). 
	 For example, X = [[1,2], 
	                   [3,4], 
	                   [5,6]] 
