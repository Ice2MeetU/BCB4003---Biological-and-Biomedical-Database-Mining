...
def group_batting(X1):
    #########################################
    ## INSERT YOUR CODE HERE (3 points)
    X2 = X1.groupby(by = 'playerID').sum().reset_index()
...
def merge_player(X2, Y):
    #########################################
    ## INSERT YOUR CODE HERE (3 points)
    X3 = X2.set_index('playerID').join(Y.set_index('playerID')).reset_index()
...
def merge_salary(X3, Z1):
    #########################################
    ## INSERT YOUR CODE HERE (3 points)
    X4 = X3.set_index('playerID').join(Z1.set_index('playerID'), lsuffix = '_x' , rsuffix = '_y').reset_index()
    X4 = X4[X4['salary'] > 0]
