...
def dataframe():
    ...
    X = pd.DataFrame(np.array([(1,4),(2,5),(3,6)]), columns=('height', 'width'))
...
def group_sum(X, k):
    #########################################
    ## INSERT YOUR CODE HERE (3 points)
    Y = X.groupby(by = [k]).sum().reset_index()
...
def merge(X, Y, k):
    #########################################
    ## INSERT YOUR CODE HERE (3 points)
    J = X.set_index(k).join(Y.set_index(k),  lsuffix = '_x' , rsuffix = '_y').dropna().reset_index()
...
def sort_values(X, k):
    #########################################
    ## INSERT YOUR CODE HERE (3 points)
    Y = X.sort_values(by = [k], axis = 0, ascending = False)
...
def insert_column(X, y, k):
    #########################################
    ## INSERT YOUR CODE HERE (3 points)
    X.insert(loc = len(X.columns), column = k, value = y)
