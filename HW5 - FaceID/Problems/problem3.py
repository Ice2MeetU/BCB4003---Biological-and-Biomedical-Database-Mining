...
'''
    ...
    In this problem, we will implement a convolutional neural network (CNN) with three convolution layers with max pooling and ReLU activations, 
    and one fully-connected layer at the end to predict the label. The classification task is that given an input gray-scale image, predict the binary label of the image 
    (e.g., whether the image is the owner of the smartphone or not). The structure of the CNN has already been defined in the 'model.py', 
    which is (Conv layer 1) -> ReLU -> maxpooling -> (Conv layer 2) -> ReLU -> maxpooling -> (Conv layer 3) -> ReLU -> maxpooling -> (Fully-connected layer)
...
def train(data_loader, alpha=0.001, n_epoch=100):
    ...
    ''' 
    If you have passed the above test, the trained CNN model should have been saved into a file, named "cnn.pt", you could run a demo by 
    typing this in the terminal: python3 demo.py
