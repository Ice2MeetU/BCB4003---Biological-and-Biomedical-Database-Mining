...
#----------------------------------------------------
'''
    (Load Game Data) Load game results from a CSV file into a pandas dataframe. Each row contains two numbers, which represents the IDs of 
    the winning team and losing team in the game in one game. 
...
#----------------------------------------------------
'''
    (compute Elo ratings) Now let's compute the elo ratings of all the NCCA teams in the dataset. Given dataframe (G) containing all the 
    game result between the teams, compute a Elo ratings of all teams. 
...
. 
    ---- Inputs: --------
        * G: a dataframe of all the game results, a pandas data frame of shape (m by 2), where each row of the matrix G[i] represents 
            the result of the i-th game, and G[i] = (A,B) contains the IDs of the winning team (A) and losing team (B) in the game.
        * n: the total number of teams to rank, an integer scalar.
        * K: k-factor, a constant number which controls how maximum amount of change that can be applied on the rating of a team 
            based upon the result of one game.
    ---- Outputs: --------
...
def compute_elo(G, n, K=16):
    ...
    R = pd.DataFrame(range(0,n), columns = ['ID'])
    R.insert(loc = len(R.columns), column = 'Elo', value = compute_ratings(G, n, K))
...
#----------------------------------------------------
'''
    If you have passed the previous test case, the result data frame should have been saved into a file, called 'ncaa_R.csv. 
    Now the dataset only contains Elo ratings, but no team name is available. We have another CSV file 'ncaa_teams.csv', 
    which contains the team names. It would be better if we can combine these two datasets into one data frame, so the new data 
    frame contains both team names and Elo ratings. 
...
#----------------------------------------------------
'''
    If you have passed the previous test case, the result data frame should have been saved into a file, called 'ncaa_X.csv'.  
    Now let's rank all of these teams using the Elo ratings in descending order. 
(Rank Teams by Elo) Given a dataframe (X) of all the teams, rank all the teams based upon descending order of Elo ratings (Elo). 
 If you have solved this problem and passed the test case, the result data frame will be saved into a file, called 'ncaa_R.csv'. 
 You could take a look at the ranking results of the teams. 
    ---- Inputs: --------
        * X: a dataframe of all the team names in NCAA merged with their Elo ratings.
    ---- Outputs: --------
        * R: a dataframe of all the team names in NCAA sorted according to the descending order of their Elo ratings.
